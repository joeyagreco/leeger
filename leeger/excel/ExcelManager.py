...
class ExcelManager:
    ...
    def __init__(self, **kwargs):
        self.__league = kwargs.pop("league", None)

        GeneralUtil.warnForUnusedKwargs(kwargs)

    def leagueToExcel(self, filePath: str, **kwargs) -> None:
        """
        If the given Excel file exists already, will raise an exception.
        """
        if self.__league is None:
        ...
        for year in self.__league.years:
            self.__yearToExcel(year, filePath, **kwargs)
        ...
        OWNER_ROW_COLORS = [getRandomColor(0.5) for _ in range(len(self.__league.owners))]
        ...
        # add all team names
        for i, owner in enumerate(self.__league.owners):
            col = "A"
        ...
        # add all stats
        statsWithTitles = self.__league.statSheet(**kwargs).preferredOrderWithTitle()
        for row, ownerId in enumerate([owner.id for owner in self.__league.owners]):
    ...
    @staticmethod
    def __yearToExcel(year: Year, filePath: str, **kwargs) -> None:
        """
        ...
        # add all stats
        statsWithTitles = year.statSheet(**kwargs).preferredOrderWithTitle()
