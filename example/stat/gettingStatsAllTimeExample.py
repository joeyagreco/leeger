...
if __name__ == "__main__":
    ...
    awalPerGame = AWALAllTimeCalculator.getAWALPerGame(
        league, yearNumberStart=2020, yearNumberEnd=2021
    )
    ...
    scoringStandardDeviation = (
        ScoringStandardDeviationAllTimeCalculator.getScoringStandardDeviation(
            league, weekNumberStart=5
        )
    )
    # Will get weeks 1-10.
    scoringStandardDeviation = (
        ScoringStandardDeviationAllTimeCalculator.getScoringStandardDeviation(
            league, weekNumberEnd=10
        )
    )
    # Will get weeks 5-10.
    scoringStandardDeviation = (
        ScoringStandardDeviationAllTimeCalculator.getScoringStandardDeviation(
            league, weekNumberStart=5, weekNumberEnd=10
        )
    )
    ...
    smartWins = SmartWinsAllTimeCalculator.getSmartWins(
        league, weekNumberStart=5, yearNumberStart=2020
    )
    # Will get week 1, 2019 - week 10, 2020.
    smartWins = SmartWinsAllTimeCalculator.getSmartWins(
        league, weekNumberEnd=10, yearNumberEnd=2020
    )
